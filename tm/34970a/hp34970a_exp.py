...
class ThermocoupleChannel(Channel):
    def __init__(self, channel, probe_type='K', unit=None):
        super(ThermocoupleChannel, self).__init(channel)
        self._probe_type = 'K'
        self._unit = unit

    def configuration(self):
        lines = ["CONF:TEMP TCOUPLE,%s,(%s)" % (self.probe_type, self.channel)]
        if self.unit is not None:
            lines += ["UNIT:TEMP %s,(%s)" % (self.unit, self.channel)]


class DCVoltageChannel(Channel):
    def __init__(self, channel, vrange=None)
        super(DCVoltageChannel, self).__init(channel)
        self.vrange = vrange

    def configuration(self):
        lines = ["CONF:VOLT:DC (%s)" % self.channel]
        if self.vrange is not None:
            lines += ["VOLT:DC:RANG %s,(%s)" % (vrange, self.channel)]
